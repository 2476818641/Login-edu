...
目标：抓一次登录，就能把 `campus-portal-auth.sh` 的认证流程一次填对。

> **现状**：脚本已按一次真实抓包（门户 `http://10.30.100.5`：三步 API + 密码 MD5）实现，
> 路径/字段/哈希方式都可配，换学校一般只改 uci 就够了。抓包现在主要用在两种情况：
> ① **你的门户和它不一样** → 按下面抓一份，让 AI 出结论，再改 uci（或脚本）；
> ② **补漏**：失败样本（故意输错密码那次）还没抓到，补一条能让失败判定从"连通性兜底"升级成精确特征串。
